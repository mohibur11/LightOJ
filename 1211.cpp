...
{
    int t,T;
    ll n,k,x1,y1,z1,x2,y2,z2,xx1,xx2,yy1,yy2,zz1,zz2;
    scin(T);
    RUN_CASE(t,T)
    {
        scln(n); cin >> x1 >> y1>>z1 >> x2 >> y2>>z2 ;
        xx1 = x1;xx2 = x2;yy1=y1;yy2=y2;zz1=z1;zz2=z2;
        for(int i=2;i<=n;i++)
        {
           cin >> x1 >> y1>>z1 >> x2 >> y2>>z2 ;
           xx1 = max(x1,xx1);yy1 = max(y1,yy1);zz1 = max(z1,zz1);
           xx2 = min(x2,xx2);yy2 = min(y2,yy2);zz2 = min(z2,zz2);

        }
        CASE(t);
        if((xx2<xx1) || (yy2<yy1)||(zz2<zz1))pf("0\n");
        else
