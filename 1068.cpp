/** DigitDP
    Approach: i construct two number, one is sum of digit % k  and number %k
    then final check is if these two number is zero or not for divisibility check
    **/
...
vi nums;
ll k;
/// pos = position of current digit
/// sum = summation of all digit of current number
/// number = main number
/// flag = for limit
ll dp[12][81][100][2];
ll call(int pos, ll sum, ll number,bool flag)
{
    if(pos == nums.size())
    {
        if(number==0 && sum==0) return 1ll;
        return 0ll;
    }
    if(dp[pos][sum][number][flag]!=-1ll) return dp[pos][sum][number][flag];
    ...
    for(int dgt = 0; dgt<=LMT; dgt++)
    {
        ...
        n_number = ((n_number * 10) + dgt)%k; /// construct number with current digit
        n_sum = (n_sum + dgt)%k;        /// add current digit

        if(flag == 0 && dgt < LMT) n_flag = 1;

        res+=call(pos+1, n_sum, n_number, n_flag);
    }

    return dp[pos][sum][number][flag] = res;
}
ll solve(ll n)
{
    nums.clear();
